...
public class MisconfigurationChecker {
    ...
    private static void checkCacheControl(String value, List<String> issues) {
        if (!value.contains("no-store")) {
            issues.add("Cache-Control missing 'no-store' —> Verify response contains no sensitive data ");
        }
        if (value.contains("public")) {
            issues.add("Cache-Control set to 'public' —>  Verify that proxy caching is intended");
        }
    }
}